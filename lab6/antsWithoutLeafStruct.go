...
func spawnLeafs(simMap Map, leafAmount int) {

	for i := 0; i < leafAmount; i++ {
		coordinate := getNewRandLeafPosition(simMap)
...
func getNewRandLeafPosition(m Map) string {
	foundNewPosition := false
	coordinate := "0.0"
	for !foundNewPosition {
		foundNewPosition = true
		...
		if value, ok := m.fields[coordinate]; ok {
			if value.ant == true || value.leaf == true {
				foundNewPosition = false
			}
		}
	}
...
func main() {
	...
	// inicjalizacja liści
	leafAmount := 40
	spawnLeafs(simMap, leafAmount)

	// symulacja (mapa, iteracje, szybkość wyświetlania)
	simulate(simMap, 1500, 1)
